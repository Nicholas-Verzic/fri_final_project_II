...
class PerspectiveTransform:
    ...
    def transformedPoint(self, x, y, angle, cy, cx):
        ...
        newPoint = transforms.functional.perspective(self.tempIMG, [[0, 0], [0, x], [y, x], [y, 0]], [[abs(y*angle), x*angle], [abs(y*angle), x-x*angle], [y-abs(y*angle), x+x*angle], [y-abs(y*angle), -x*angle]], fill = (self.fillColor))
    ...
    def ellipseTransform(self, x, y, angle, ellipseRegions):
        for i in range(len(ellipseRegions)):
            ...
            (cy, cx) = self.transformedPoint(x, y, angle, ellipseRegions[i]["shape_attributes"]["cy"], ellipseRegions[i]["shape_attributes"]["cx"])
            ellipseRegions[i]["shape_attributes"]["rx"] = math.dist(self.transformedPoint(x, y, angle, ellipseRegions[i]["shape_attributes"]["cy"] + (soh * ellipseRegions[i]["shape_attributes"]["rx"]), ellipseRegions[i]["shape_attributes"]["cx"] + (cah * ellipseRegions[i]["shape_attributes"]["rx"])), (cy, cx))
            ellipseRegions[i]["shape_attributes"]["ry"] = math.dist(self.transformedPoint(x, y, angle, ellipseRegions[i]["shape_attributes"]["cy"] + (cah * ellipseRegions[i]["shape_attributes"]["ry"]), ellipseRegions[i]["shape_attributes"]["cx"] + (soh * ellipseRegions[i]["shape_attributes"]["ry"])), (cy, cx))
            (ellipseRegions[i]["shape_attributes"]["cy"], ellipseRegions[i]["shape_attributes"]["cx"]) = (cy, cx)
    ...
    def circleTransform(self, x, y, angle, circleRegions):
        for i in range(len(circleRegions)):
            ...
            (cy, cx) = self.transformedPoint(x, y, angle, circleRegions[i]["shape_attributes"]["cy"], circleRegions[i]["shape_attributes"]["cx"])
            circleRegions[i]["shape_attributes"]["rx"] = self.transformedPoint(x, y, angle, circleRegions[i]["shape_attributes"]["cy"], circleRegions[i]["shape_attributes"]["cx"] + circleRegions[i]["shape_attributes"]["r"])[1] - cx
            circleRegions[i]["shape_attributes"]["ry"] = self.transformedPoint(x, y, angle, circleRegions[i]["shape_attributes"]["cy"] + circleRegions[i]["shape_attributes"]["r"], circleRegions[i]["shape_attributes"]["cx"])[0] - cy
            (circleRegions[i]["shape_attributes"]["cy"], circleRegions[i]["shape_attributes"]["cx"]) = (cy, cx)
    ...
    def polyTransform(self, x, y, angle, polyRegions):
        for i in range(len(polyRegions)):
            for j in range(len(polyRegions[i]["shape_attributes"]["all_points_x"])):
                (polyRegions[i]["shape_attributes"]["all_points_y"][j], polyRegions[i]["shape_attributes"]["all_points_x"][j]) = self.transformedPoint(x, y, angle, polyRegions[i]["shape_attributes"]["all_points_y"][j], polyRegions[i]["shape_attributes"]["all_points_x"][j])
    ...
    def rectTransform(self, x, y, angle, rectRegions):
        for i in range(len(rectRegions)):
            ...
            pointList.append(self.transformedPoint(x, y, angle, rectRegions[i]["shape_attributes"]["y"], rectRegions[i]["shape_attributes"]["x"]))
            pointList.append(self.transformedPoint(x, y, angle, rectRegions[i]["shape_attributes"]["y"]+rectRegions[i]["shape_attributes"]["height"], rectRegions[i]["shape_attributes"]["x"]+rectRegions[i]["shape_attributes"]["width"]))
            pointList.append(self.transformedPoint(x, y, angle, rectRegions[i]["shape_attributes"]["y"], rectRegions[i]["shape_attributes"]["x"]+rectRegions[i]["shape_attributes"]["width"]))
            pointList.append(self.transformedPoint(x, y, angle, rectRegions[i]["shape_attributes"]["y"]+rectRegions[i]["shape_attributes"]["height"], rectRegions[i]["shape_attributes"]["x"]))
    ...
    def __call__(self, val, path, imgName, image):
        ...
        for i, angle in enumerate(self.angles):
            ...
            try:
                ...
                tensorIMG = transforms.functional.perspective(image, [[0, 0], [0, x], [y, x], [y, 0]], [[abs(y*angle), x*angle], [abs(y*angle), x-x*angle], [y-abs(y*angle), x+x*angle], [y-abs(y*angle), -x*angle]], fill = (self.fillColor))
                ...
                newShape = newShape + self.ellipseTransform(x, y, angle, copy.deepcopy(ellipseRegions))
                print("Circle Transform")
                newShape = newShape + self.circleTransform(x, y, angle, copy.deepcopy(circleRegions))
